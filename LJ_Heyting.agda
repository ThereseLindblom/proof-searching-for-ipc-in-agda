...
module Soundness
  ...
  ∧h-≤-∨h : (a b c : H) → (a ∧h b) ≤ (b ∨h c)
  ∧h-≤-∨h a b c = ≤-trans (a ∧h b) b (b ∨h c) (H3 a b) (H6 b c)

  ∧h-d-∨h-lemma₁ : (a b c : H) → ((b ∧h a) ∨h (c ∧h a)) ≤ ((b ∨h c) ∧h a)
  ∧h-d-∨h-lemma₁ a b c =
                  H8
                    (b ∧h a) (c ∧h a) ((b ∨h c) ∧h a)
                    (H4
                      (b ∨h c) a (b ∧h a)
                      (≤-trans
                        (b ∧h a) b (b ∨h c)
  ...
  ∧h-d-∨h-lemma₂ : (a b c : H) → ((b ∨h c) ∧h a) ≤ ((b ∧h a) ∨h (c ∧h a))
  ∧h-d-∨h-lemma₂ a b c = H9₂ a ((b ∧h a) ∨h (c ∧h a)) (b ∨h c)
                             (H8 b c (a ⟶h ((b ∧h a) ∨h (c ∧h a)))
                               (H9₁ a ((b ∧h a) ∨h (c ∧h a)) b
                                 (H6 (b ∧h a) (c ∧h a)))
                               (H9₁ a ((b ∧h a) ∨h (c ∧h a)) c
                                 (H7 (b ∧h a) (c ∧h a))
                               )
                             )

  ∧h-distrib-∨h : (a b c : H) → (b ∨h c) ∧h a ≡ (b ∧h a) ∨h (c ∧h a)
  ∧h-distrib-∨h a b c = ≤-antisym ((b ∨h c) ∧h a) ((b ∧h a) ∨h (c ∧h a))
                                  {!∧h-d-∨h-lemma₂ a b c!}
                                  (∧h-d-∨h-lemma₁ a b c)
  
  ∨h-distrib-∧h : (a b c : H) → a ∨h (b ∧h c) ≡ (a ∨h b) ∧h (a ∨h c)
  ∨h-distrib-∧h a b c = ≤-antisym (a ∨h (b ∧h c)) ((a ∨h b) ∧h (a ∨h c))
                                  {!!}
                                  {!!}
  ...
  Γ ⊨ C  = ∀ {a : Assignment} → a satisfies Γ → (〚 sumΓ Γ 〛 a) ≤ (〚 C 〛 a)
  ...
  --≤ as implication istället för val-dec

  -- The final boss
  
  soundness : ∀ {Γ} {C} → Γ ⊢ C → Γ ⊨ C
  soundness (id x) a_sat_Γ = {!!}
  soundness ⊤R a_sat_Γ = {!!}
  soundness (⊥L x) a_sat_Γ = {!!}
  soundness (∧R d d₁) a_sat_Γ = {!!}
